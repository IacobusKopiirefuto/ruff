...
impl<'db, 'ast> TypeInferenceBuilder<'db, 'ast> {
    ...
    fn infer_newtype_expression(
        ...
        // REVIEWERS: Is it worth trying to do full argument parsing here? If so is there some
        // machinery we might want to reuse or factor out somewhere else?
        if let Some(starred) = arguments.args.iter().find(|arg| arg.is_starred_expr()) {
            return error(
                &self.context,
    ...
    fn infer_typevar_assignment_deferred(&mut self, arguments: &ast::Arguments) {
